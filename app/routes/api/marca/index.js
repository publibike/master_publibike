...
module.exports.register = async server => {
    ...
    server.route({
        ...
        handler: async (req, h) => {
            ...
            try {
                ...
                const data = await req.mongo.db.collection('Administrador').findOne({ "usuario": usuario });
    ...
    server.route({
        ...
        handler: async (req, h) => {
            ...
            const admin = req.mongo.db.collection('Administrador').find({ _id: new ObjectID(id) });
            console.log(admin)
            return admin;
        }
    });
    ...
    server.route({
        ...
        handler: async (req, h) => {
            ...
            const status = await req.mongo.db.collection('Usuario').insertOne(payload);
        ...
        handler: async (req, h) => {
            ...
            try {
                ...
                if (Buffer.isBuffer(Buffer.from(req.payload.imagen))) {
                    ...
                    filename = `${uuidv1()}.png`;
                    await write(join(__dirname, '../../../..', 'public', 'uploads', filename), cam.imagen)
                }
                ...
                const stsEmpresa = await req.mongo.db.collection('Empresa').updateOne({ _id: new ObjectID(id) }, { $push: { campanas: camEmp } })
    ...
    server.route({
        ...
        handler: async (req, h) => {
            ...
            try {
                ...
                const data = await req.mongo.db.collection('Usuario').findOne({ "email": us.email });
    ...
    server.route({
        ...
        handler: async (req, h) => {
            try {
                const campanas = await req.mongo.db.collection('Campaña').find({}).toArray();
                return campanas
    ...
    server.route({
        ...
        handler: async (req, h) => {
            try {
                ...
                const campana = req.mongo.db.collection('Campaña').findOne({ _id: new ObjectID(id) });
                return campana;
            } catch (error) {
                console.log(error)
            }
    ...
    server.route({
        ...
        handler: async (req, h) => {
            try {
                const empresa = await req.mongo.db.collection('Empresa').find({}).toArray();
                return empresa
    ...
    server.route({
        ...
        handler: async (req, h) => {
            ...
            try {
                ...
                const status = await req.mongo.db.collection('Usuario').updateOne({ _id: ObjectID(id) }, { $set: payload });
            } catch (error) {
                return error
            }
        ...
        handler: async (req, h) => {
            try {
              ...
              //Actualiza los totales del usuario
                const status = await req.mongo.db.collection('Usuario').updateOne({ _id: ObjectID(id) }, { $push: { recorridos: payload }, $set: { km_total: user.km_total, co2_total: user.co2_total, cal_total: user.cal_total, tiempo_total: user.tiempo_total, ganancia_total: user.ganancia_total } });

                //Obtiene la campaña actual
                let camActual = await req.mongo.db.collection('Campaña').findOne({ _id: new ObjectID(camActId) })
                ...
                for (let i = 0; i < camActual.usuarios.length; i++) {
                    ...
                    if (idUsuario === idUsCampana) {
                        ...
                        const statusCampanaUser = await req.mongo.db.collection('Usuario').updateOne({ _id: ObjectID(id), 'campana.id': camActId }, { $set: { kms: element.kms, co2: element.co2, cal: element.cal, tiempo: element.tiempo, ingresos: element.ingresos } });
                        const statusCampanaActUser = await req.mongo.db.collection('Usuario').updateOne({ _id: ObjectID(id), 'campana_actual.id': camActId }, { $set: { kms: element.kms, co2: element.co2, cal: element.cal, tiempo: element.tiempo, ingresos: element.ingresos } });


                        const statusUserCampana = await req.mongo.db.collection('Campaña').updateOne({ _id: camActId, 'usuarios.id': ObjectID(id) }, { $set: { kms: element.kms, co2: element.co2, cal: element.cal, tiempo: element.tiempo, ingresos: element.ingresos } });

                        const statusCampana = await req.mongo.db.collection('Campaña').updateOne({ _id: camActId }, { $set: { km_total: camActual.km_total, co2_total: camActual.co2_total, cal_total: camActual.cal_total, tiempo_total: camActual.tiempo_total } });
    ...
    server.route({
        ...
        handler: async (req, h) => {
            try {
                ...
                const ObjectID = req.mongo.ObjectID;
                const payload = req.payload
                const usuario = await req.mongo.db.collection('Usuario').findOne({ _id: ObjectID(id) });
                let campana = await req.mongo.db.collection('Campaña').findOne({ _id: ObjectID(payload.id) });
                // console.log(campana)
                if (campana.cupos != 0) {
                    console.log("1")
                    ...
                    for (let index = 0; index < campana.usuarios.length + 1; index++) {
                        ...
                        if (element) {
                            ...
                            if (element.id.equals(usuario._id)) {
                                ...
                                payload.tiempo = 0
                                payload.ingreso = 0
                                const statusUsuario = await req.mongo.db.collection('Usuario').updateOne({ _id: ObjectID(id) }, { $push: { campanas: payload } });
                                const statusCampana = await req.mongo.db.collection('Campaña').updateOne({ _id: payload.id }, { $push: { usuarios: usCampana }, $set: { cupos: y } });
    ...
    server.route({
        ...
        handler: async (req, h) => {
            try {
                ...
                console.log(payload)
                payload.id = ObjectID(payload.id)
                const statusUsuario = await req.mongo.db.collection('Usuario').updateOne({ _id: ObjectID(id) }, { $set: { campana_actual: payload } });
